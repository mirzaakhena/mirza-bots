...
describe("resolveSocketPath", () => {
  test("honors MIRZA_BOTS_HOME override, same convention as fleetd", () => {
    ...
    try {
      // Built with join() rather than a "/"-separated literal: what this test is
      // about is the location and the convention, not the separator, and on
      // Windows the implementation's own join() yields backslashes.
      expect(resolveSocketPath()).toBe(join("/tmp/fake-mirza-bots", "fleetd.sock"));
    } finally {
  ...
  test("falls back to ~/.claude/mirza-bots/fleetd.sock when unset", () => {
    ...
    try {
      ...
      expect(resolveSocketPath()).toBe(join(home, ".claude", "mirza-bots", "fleetd.sock"));
    } finally {
